...
export type {ContributionKind, ContributionLabel} from './types';
...
export class ScoreCalculator {
  ...
  static buildIssuePrData(repo: DetailedRepoData): IssuePrData[] {
    ...
    for (const issue of repo.issues) {
      ...
      const target = getOrCreate(issue.author ?? UNKNOWN_USER);
    ...
    for (const pr of repo.prs) {
      ...
      const target = getOrCreate(pr.author ?? UNKNOWN_USER);
  ...
  static calculateRepoData(repo: DetailedRepoData): RepoData {
    return {
      owner: repo.owner,
      repo: repo.repo,
      scoreData: ScoreCalculator.buildIssuePrData(repo),
    };
  }
